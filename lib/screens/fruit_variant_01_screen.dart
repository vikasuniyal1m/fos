...
/// Fruit Variant 01 Screen
/// Displays only fruit with _01 in their code, shown prominently on the front
class FruitVariant01Screen extends StatefulWidget {
  const FruitVariant01Screen({Key? key}) : super(key: key);

  @override
  State<FruitVariant01Screen> createState() => _FruitVariant01ScreenState();
}

class _FruitVariant01ScreenState extends State<FruitVariant01Screen> {
  final controller = Get.find<FruitController>();
  final homeController = Get.find<HomeController>();
  var fruitEmoji01 = <Map<String, dynamic>>[];
  ...
  Future<void> _loadFruitEmoji01() async {
    try {
      ...
      print('✅ Loaded ${emojis.length} unique _01 variant fruit');
      for (var fruit in emojis) {
        print('   - ${fruit['name']}: ${fruit['code']}');
      }
      
      // If no fruit found, create temporary fruit emoji with _01 variants
      if (emojis.isEmpty) {
        print('⚠️ No _01 fruit emoji found in database, creating temporary ones...');
        emojis = _createTemporaryFruitEmoji01();
      }
      ...
      print('Error loading fruit emoji _01: $e');
      // Create temporary emoji even on error
      final tempEmojis = _createTemporaryFruitEmoji01();
      setState(() {
        fruitEmoji01 = tempEmojis;
        isLoadingEmojis = false;
      });
    }
  }

  List<Map<String, dynamic>> _createTemporaryFruitEmoji01() {
              ...
              if (isLoadingEmojis)
                  ...
                  child: Center(
                    child: Column(
                      children: [
                        ...
                        Text(
                          'No _01 variant fruit available',
                          style: ResponsiveHelper.textStyle(
                            context,
                ...
                Padding(
                  ...
                  child: GridView.builder(
                    ...
                    itemCount: fruitEmoji01.length,
                    itemBuilder: (context, index) {
                      final fruitEmoji = fruitEmoji01[index];
